...
module.exports = function(config) {
    ...
    const instance = {
        ...
        getTransactionByID(user, tx_id) {
            return this.get_member_user(user)
                .then(user_from_store => {
                    return channel.queryTransaction(tx_id);
                })
                .then(query_responses => {
                    if (query_responses) {
                        if (query_responses.transactionEnvelope == null) {
                            console.error("error from query = ", query_responses);
                        } else {
                            return query_responses.transactionEnvelope.payload;
                        }
                    } else {
                        console.log("No payloads were returned from query");
                        return null;
